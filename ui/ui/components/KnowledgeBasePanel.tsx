...
// ---------------------------------------------------------------------------
// Static content — Security + Product docs hub
// Explains how CyberOracle works and what it blocks.
// ---------------------------------------------------------------------------
...
const ARTICLES: Article[] = [
  {
    ...
    content: (
      <div className="space-y-3 text-xs text-slate-600 leading-relaxed">
        <p>
          <strong className="text-slate-800">CyberOracle</strong> is a Secure AI
        ...
        <p>
          ...
          <strong>redacted</strong> before reaching the model or{' '}
          <strong>blocked</strong> entirely, depending on the configured policy.
        ...
        <div className="rounded-lg bg-sky-50 border border-sky-100 px-4 py-3 mt-2">
          <p className="font-semibold text-sky-800 mb-1">Capstone I scope</p>
  ...
  },
  {
    ...
    content: (
      <div className="space-y-4 text-xs text-slate-600 leading-relaxed">
        ...
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 font-mono text-[11px] text-slate-700 leading-6">
          <p>Browser / App</p>
          <p className="pl-4">↓  POST /ai/query &#123;prompt&#125;</p>
          <p>Rate Limiter middleware</p>
          <p className="pl-4">↓</p>
          <p>DLP Filter middleware (input scan)</p>
          <p className="pl-4">↓  BLOCK if critical PII detected</p>
          <p className="pl-4">↓  REDACT and continue otherwise</p>
          <p>Ollama LLM  (local)</p>
          <p className="pl-4">↓  raw response</p>
          <p>DLP Filter middleware (output scan)</p>
          <p className="pl-4">↓  BLOCK / REDACT / ALLOW</p>
          <p>Structured logger → PostgreSQL (encrypted)</p>
          <p className="pl-4">↓</p>
          <p>Alert Manager → Discord / Slack  (on high-risk events)</p>
          <p className="pl-4">↓</p>
          <p>Response returned to caller</p>
        </div>
        <p>
          The <strong>Circuit Breaker</strong> pattern wraps the Ollama client —
          if the model is unavailable, requests fail fast rather than piling up.
          A <strong>Model Router</strong> abstracts the LLM provider so future
  ...
  },
  {
    ...
    content: (
      <div className="space-y-4 text-xs text-slate-600 leading-relaxed">
        ...
        <h3 className="text-xs font-semibold text-slate-800 mt-2">
          Regex DLP — Pattern matching
        </h3>
        <table className="w-full text-xs border-collapse">
          <thead>
            <tr className="bg-slate-50 text-left">
              <th className="px-3 py-2 border border-slate-200 font-semibold text-slate-700">
                Pattern
              </th>
              <th className="px-3 py-2 border border-slate-200 font-semibold text-slate-700">
                Severity
          ...
          <tbody>
            {[
              ...
              <tr key={pattern} className="border-b border-slate-100">
                <td className="px-3 py-2 border border-slate-100">{pattern}</td>
                ...
                <td className="px-3 py-2 border border-slate-100 font-mono text-slate-500">
                  {ex}
                </td>
              </tr>
            ))}
        ...
        <p>
          ...
          them with type labels (e.g. <code className="font-mono bg-slate-100 px-1 rounded">&lt;PERSON&gt;</code>).
        </p>
        ...
        <div className="space-y-1">
          {[
            ['Allow', 'green', 'No sensitive data detected — request proceeds unchanged.'],
    ...
    content: (
      <div className="space-y-4 text-xs text-slate-600 leading-relaxed">
        ...
        {[
          ...
          >
            <p className="font-semibold text-slate-800 mb-1">{f.name}</p>
            <p>{f.desc}</p>
          </div>
        ))}
    ...
    content: (
      <div className="space-y-4 text-xs text-slate-600 leading-relaxed">
        ...
        <div className="rounded-lg bg-amber-50 border border-amber-200 px-4 py-3">
          <p className="font-semibold text-amber-800 mb-1">Capstone I status</p>
          <p className="text-amber-700">
            The RBAC dependency module is implemented in{' '}
            <code className="font-mono text-[11px]">app/auth/rbac.py</code> and
  ...
  },
  {
    ...
    content: (
      <div className="space-y-4 text-xs text-slate-600 leading-relaxed">
        ...
        <h3 className="text-xs font-semibold text-slate-800">Alert triggers</h3>
        <div className="space-y-2">
          {[
            ...
            <div key={name as string} className="rounded-lg border border-slate-200 p-3">
              <div className="flex items-center gap-2 mb-1">
                <span className="font-semibold text-slate-800">{name}</span>
                <span
                  className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${
                    sev === 'Critical' ? 'bg-red-100 text-red-700' : sev === 'High' ? 'bg-orange-100 text-orange-700' : 'bg-yellow-100 text-yellow-700'
                  }`}
                >
                  {sev}
                </span>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 rounded-lg border border-slate-200 p-3">
            ...
            <span>
              Configured via <code className="font-mono bg-slate-100 px-1 rounded">DISCORD_WEBHOOK_URL</code> in{' '}
          ...
          <div className="flex items-center gap-2 rounded-lg border border-slate-200 p-3">
            ...
            <span>
              Configured via{' '}
              <code className="font-mono bg-slate-100 px-1 rounded">SLACK_WEBHOOK_URL</code> in{' '}
              <code className="font-mono bg-slate-100 px-1 rounded">.env</code>.
  ...
  },
  {
    ...
    content: (
      <div className="space-y-3 text-xs text-slate-600">
        <p>All endpoints are served at <code className="font-mono bg-slate-100 px-1 rounded">http://localhost:8000</code> by default.</p>
        <table className="w-full text-xs border-collapse">
          <thead>
            <tr className="bg-slate-50 text-left">
              <th className="px-3 py-2 border border-slate-200 font-semibold text-slate-700">Method</th>
              <th className="px-3 py-2 border border-slate-200 font-semibold text-slate-700">Path</th>
              <th className="px-3 py-2 border border-slate-200 font-semibold text-slate-700">Description</th>
            </tr>
          </thead>
          <tbody>
            {[
                ...
                <td className="px-3 py-2 border border-slate-100">
                  <span
                    className={`font-mono font-semibold ${
                      method === 'POST' ? 'text-sky-700' : 'text-green-700'
                    }`}
                  >
                    {method}
                  </span>
        ...
        <p className="text-slate-400">
          Full interactive docs: <code className="font-mono bg-slate-100 px-1 rounded">http://localhost:8000/docs</code>
        </p>
      </div>
    ),
  },
  ...
  return (
    <div className="mt-4">
      ...
      <div className="flex gap-5">
        ...
        {/* Content */}
        <article className="flex-1 bg-white rounded-xl border border-slate-200 shadow-sm p-6 min-h-[400px]">
