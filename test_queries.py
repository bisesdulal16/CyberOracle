...
async def test_queries():
    async with AsyncSessionLocal() as session:
        ...
        result = await session.execute(
            text(
                "SELECT policy_decision AS label, COUNT(*)::bigint AS value FROM logs WHERE created_at >= NOW() - INTERVAL '24 hours' GROUP BY policy_decision"
            )
        )
        ...
        result = await session.execute(
            text(
                "SELECT date_trunc('minute', created_at) AS time, COUNT(*)::bigint AS value FROM logs WHERE policy_decision IN ('allow', 'redact', 'block') AND created_at >= NOW() - INTERVAL '24 hours' GROUP BY 1 ORDER BY 1 LIMIT 10"
            )
        )
        ...
        result = await session.execute(
            text(
                "SELECT date_trunc('minute', created_at) AS time, COUNT(*)::bigint AS value FROM logs WHERE frameworks LIKE '%HIPAA%' AND created_at >= NOW() - INTERVAL '24 hours' GROUP BY 1 ORDER BY 1 LIMIT 10"
            )
        )
        ...
        result = await session.execute(
            text(
                "SELECT date_trunc('minute', created_at) AS time, COUNT(*)::bigint AS value FROM logs WHERE frameworks LIKE '%FERPA%' AND created_at >= NOW() - INTERVAL '24 hours' GROUP BY 1 ORDER BY 1 LIMIT 10"
            )
        )
        ...
        result = await session.execute(
            text(
                "SELECT date_trunc('minute', created_at) AS time, COUNT(*)::bigint AS value FROM logs WHERE severity = 'high' AND created_at >= NOW() - INTERVAL '24 hours' GROUP BY 1 ORDER BY 1 LIMIT 10"
            )
        )
        ...
        result = await session.execute(
            select(LogEntry)
            .where(LogEntry.created_at >= text("NOW() - INTERVAL '24 hours'"))
        ...
        for e in recent:
            print(
                f"  ID={e.id}, endpoint={e.endpoint}, policy_decision={e.policy_decision}, severity={e.severity}, frameworks={e.frameworks}"
            )
